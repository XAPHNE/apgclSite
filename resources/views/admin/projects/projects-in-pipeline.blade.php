...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Projects in Pipeline</li>
@endsection
    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Projects in Pipeline List</h3>
            ...
            <div class="card-body">
                <table id="table" class="table display compact table-bordered table-hover" style="width: 100%">
                    ...
                    <tbody>
                        @foreach ($projectsInPipelines as $projectsInPipeline)
                            <tr>
                                <td class="text-center align-middle">{{ $loop->iteration }}</td>
                                <td class="text-center align-middle">{{ $projectsInPipeline->name }}</td>
                                <td class="text-center align-middle">{{ $projectsInPipeline->capacity }}</td>
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $projectsInPipeline->id }}"
                                        data-name="{{ $projectsInPipeline->name }}"
                                        data-capacity="{{ $projectsInPipeline->capacity }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $projectsInPipeline->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Project In Pipeline</h5>
...
<div class="modal fade" id="deleteConfirmationModal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this Project in Pipeline?</p>
            </div>
            <div class="modal-footer justify-content-between">
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Project In Pipeline');
            $('#addUpdateForm').attr('action', '{{ route('projects-in-pipeline.store') }}');
        ...
        $(document).on('click', '.update-button', function () {
            ...
            $('#addUpdateForm').attr('action', '/admin/projects/projects-in-pipeline/' + id);
        ...
        $(document).on('click', '.delete-button', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/projects/projects-in-pipeline/' + id;
