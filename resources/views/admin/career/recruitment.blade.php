    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Recruitment List</h3>
                    ...
                    <tbody>
                        @foreach ($recruitments as $recruitment)
                            <tr>
                                ...
                                <td class="text-center align-middle">{{ $recruitment->name }}</td>
                                <td class="text-center align-middle">{{ $recruitment->description }}</td>
                                <td class="text-center align-middle">
                                    @if ($recruitment->visibility)
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($recruitment->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $recruitment->id }}"
                                        data-name="{{ $recruitment->name }}"
                                        data-description="{{ $recruitment->description }}"
                                        data-downloadlink="{{ $recruitment->downloadLink }}"
                                        data-visibility="{{ $recruitment->visibility }}"
                                        data-news_n_events="{{ $recruitment->news_n_events }}"
                                        data-new_badge="{{ $recruitment->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $recruitment->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Recruitment</h5>
...
<div class="modal fade" id="deleteConfirmationModal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this Recruitment?</p>
            </div>
            <div class="modal-footer justify-content-between">
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Recruitment');
            $('#addUpdateForm').attr('action', '{{ route('recruitments.store') }}');
        ...
        $(document).on('click', '.update-button', function () {
            ...
            $('#modalTitle').text('Update Recruitment');
            $('#addUpdateForm').attr('action', '/admin/career/recruitments/' + id);
        ...
        $(document).on('click', '.delete-button', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/career/recruitments/' + id;
