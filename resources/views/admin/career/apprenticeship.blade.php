...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Apprenticeship</li>
@endsection
    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Apprenticeship List</h3>
                    ...
                    <tbody>
                        @foreach ($apprenticeships as $apprenticeship)
                            <tr>
                                ...
                                <td class="text-center align-middle">{{ $apprenticeship->name }}</td>
                                <td class="text-center align-middle">{{ $apprenticeship->description }}</td>
                                <td class="text-center align-middle">
                                    @if ($apprenticeship->visibility)
                                ...
                                <td class="text-center align-middle">
                                    @if ($apprenticeship->news_n_events)
                                        <i class="fas fa-check-circle text-success"></i>
                                    @else
                                        <i class="fas fa-times-circle text-danger"></i>
                                    @endif
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($apprenticeship->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $apprenticeship->id }}"
                                        data-name="{{ $apprenticeship->name }}"
                                        data-description="{{ $apprenticeship->description }}"
                                        data-downloadlink="{{ $apprenticeship->downloadLink }}"
                                        data-visibility="{{ $apprenticeship->visibility }}"
                                        data-news_n_events="{{ $apprenticeship->news_n_events }}"
                                        data-new_badge="{{ $apprenticeship->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $apprenticeship->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Apprenticeship</h5>
...
<div class="modal fade" id="deleteConfirmationModal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this apprenticeship?</p>
            </div>
            <div class="modal-footer justify-content-between">
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Apprenticeship');
            $('#addUpdateForm').attr('action', '{{ route('apprenticeship.store') }}');
        ...
        $('.update-button').on('click', function () {
            ...
            $('#modalTitle').text('Update Apprenticeship');
            $('#addUpdateForm').attr('action', '/admin/career/apprenticeship/' + id);
        ...
        $('.delete-button').on('click', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/career/apprenticeship/' + id;
