                    ...
                    <tbody>
                        @foreach ($csrs as $csr)
                            <tr>
                                ...
                                <td class="text-center align-middle">{{ $csr->name }}</td>
                                <td class="text-center align-middle">{{ $csr->description }}</td>
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($csr->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $csr->id }}"
                                        data-name="{{ $csr->name }}"
                                        data-description="{{ $csr->description }}"
                                        data-downloadlink="{{ $csr->downloadLink }}"
                                        data-visibility="{{ $csr->visibility }}"
                                        data-news_n_events="{{ $csr->news_n_events }}"
                                        data-new_badge="{{ $csr->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $csr->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New CSR</h5>
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New CSR');
            $('#addUpdateForm').attr('action', '{{ route('corporate-social-responsibility.store') }}');
        ...
        $('.update-button').on('click', function () {
            ...
            $('#modalTitle').text('Update CSR');
            $('#addUpdateForm').attr('action', '/admin/corporate-social-responsibility/' + id);
