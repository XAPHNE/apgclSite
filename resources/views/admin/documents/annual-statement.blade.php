...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Annual Statements</li>
@endsection
    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Annual Statements List</h3>
                    ...
                    <tbody>
                        @foreach ($annualStatements as $annualStatement)
                            <tr>
                                ...
                                <td class="text-center align-middle">{{ $annualStatement->name }}</td>
                                <td class="text-center align-middle">{{ $annualStatement->description }}</td>
                                ...
                                <td class="text-center align-middle">
                                    @if ($annualStatement->news_n_events)
                                        <i class="fas fa-check-circle text-success"></i>
                                    @else
                                        <i class="fas fa-times-circle text-danger"></i>
                                    @endif
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($annualStatement->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $annualStatement->id }}"
                                        data-name="{{ $annualStatement->name }}"
                                        data-description="{{ $annualStatement->description }}"
                                        data-downloadlink="{{ $annualStatement->downloadLink }}"
                                        data-visibility="{{ $annualStatement->visibility }}"
                                        data-news_n_events="{{ $annualStatement->news_n_events }}"
                                        data-new_badge="{{ $annualStatement->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $annualStatement->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Annual Statement</h5>
...
<div class="modal fade" id="deleteConfirmationModal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this annual statement?</p>
            </div>
            <div class="modal-footer justify-content-between">
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Annual Statement');
            $('#addUpdateForm').attr('action', '{{ route('annual-statements.store') }}');
        ...
        $('.update-button').on('click', function () {
            ...
            $('#modalTitle').text('Update Annual Statement');
            $('#addUpdateForm').attr('action', '/admin/documents/annual-statements/' + id);
        ...
        $('.delete-button').on('click', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/documents/annual-statements/' + id;
