...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Service Rules</li>
@endsection
    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Service Rules List</h3>
            ...
            <div class="card-body">
                <table id="table" class="table display compact table-bordered table-hover" style="width: 100%">
                    ...
                    <tbody>
                        @foreach ($serviceRules as $serviceRule)
                            <tr>
                                <td class="text-center align-middle">{{ $loop->iteration }}</td>
                                <td class="text-center align-middle">{{ $serviceRule->name }}</td>
                                <td class="text-center align-middle">{{ $serviceRule->description }}</td>
                                <td class="text-center align-middle">
                                    @if ($serviceRule->visibility)
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($serviceRule->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $serviceRule->id }}"
                                        data-name="{{ $serviceRule->name }}"
                                        data-description="{{ $serviceRule->description }}"
                                        data-downloadlink="{{ $serviceRule->downloadLink }}"
                                        data-visibility="{{ $serviceRule->visibility }}"
                                        data-news_n_events="{{ $serviceRule->news_n_events }}"
                                        data-new_badge="{{ $serviceRule->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $serviceRule->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Service Rule</h5>
...
<div class="modal fade" id="deleteConfirmationModal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this service rule?</p>
            </div>
            <div class="modal-footer justify-content-between">
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Service Rule');
            $('#addUpdateForm').attr('action', '{{ route('service-rules.store') }}');
        ...
        $('.update-button').on('click', function () {
            ...
            $('#modalTitle').text('Update Service Rule');
            $('#addUpdateForm').attr('action', '/admin/documents/service-rules/' + id);
        ...
        $('.delete-button').on('click', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/documents/service-rules/' + id;
