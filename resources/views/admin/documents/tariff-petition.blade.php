...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Tariff Petitions</li>
@endsection
    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Tariff Petition List</h3>
            ...
            <div class="card-body">
                <table id="table" class="table display compact table-bordered table-hover" style="width: 100%">
                    ...
                    <tbody>
                        @foreach ($tariffPetitions as $tariffPetition)
                            <tr>
                                <td class="text-center align-middle">{{ $loop->iteration }}</td>
                                <td class="text-center align-middle">{{ $tariffPetition->name }}</td>
                                <td class="text-center align-middle">{{ $tariffPetition->description }}</td>
                                <td class="text-center align-middle">
                                    @if ($tariffPetition->visibility)
                                ...
                                <td class="text-center align-middle">
                                    @if ($tariffPetition->news_n_events)
                                        <i class="fas fa-check-circle text-success"></i>
                                    @else
                                        <i class="fas fa-times-circle text-danger"></i>
                                    @endif
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($tariffPetition->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $tariffPetition->id }}"
                                        data-name="{{ $tariffPetition->name }}"
                                        data-description="{{ $tariffPetition->description }}"
                                        data-downloadlink="{{ $tariffPetition->downloadLink }}"
                                        data-visibility="{{ $tariffPetition->visibility }}"
                                        data-news_n_events="{{ $tariffPetition->news_n_events }}"
                                        data-new_badge="{{ $tariffPetition->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $tariffPetition->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Tariff Petition</h5>
...
<div class="modal fade" id="deleteConfirmationModal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this Tariff Petition?</p>
            </div>
            <div class="modal-footer justify-content-between">
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Tariff Petition');
            $('#addUpdateForm').attr('action', '{{ route('tariff-petition.store') }}');
        ...
        $('.update-button').on('click', function () {
            ...
            $('#modalTitle').text('Update Tariff Petition');
            $('#addUpdateForm').attr('action', '/admin/documents/tariff-petition/' + id);
        ...
        $('.delete-button').on('click', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/documents/tariff-petition/' + id;
