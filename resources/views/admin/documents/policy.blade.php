    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Policies List</h3>
                    ...
                    <tbody>
                        @foreach ($policies as $policy)
                            <tr>
                                ...
                                <td class="text-center align-middle">{{ $policy->name }}</td>
                                <td class="text-center align-middle">{{ $policy->description }}</td>
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($policy->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $policy->id }}"
                                        data-name="{{ $policy->name }}"
                                        data-description="{{ $policy->description }}"
                                        data-downloadlink="{{ $policy->downloadLink }}"
                                        data-visibility="{{ $policy->visibility }}"
                                        data-news_n_events="{{ $policy->news_n_events }}"
                                        data-new_badge="{{ $policy->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $policy->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Policy</h5>
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            ...
            $('#addUpdateForm').attr('action', '{{ route('policies.store') }}');
        ...
        $(document).on('click', '.update-button', function () {
            ...
            $('#modalTitle').text('Update Policy');
            $('#addUpdateForm').attr('action', '/admin/documents/policies/' + id);
        ...
        $(document).on('click', '.delete-button', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/documents/policies/' + id;
