    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Certificates List</h3>
                    ...
                    <tbody>
                        @foreach ($certificates as $certificate)
                            <tr>
                                ...
                                <td class="text-center align-middle">{{ $certificate->name }}</td>
                                <td class="text-center align-middle">{{ $certificate->description }}</td>
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($certificate->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $certificate->id }}"
                                        data-name="{{ $certificate->name }}"
                                        data-description="{{ $certificate->description }}"
                                        data-downloadlink="{{ $certificate->downloadLink }}"
                                        data-visibility="{{ $certificate->visibility }}"
                                        data-news_n_events="{{ $certificate->news_n_events }}"
                                        data-new_badge="{{ $certificate->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $certificate->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            ...
            $('#addUpdateForm').attr('action', '{{ route('certificates.store') }}');
        ...
        $('.update-button').on('click', function () {
            ...
            $('#addUpdateForm').attr('action', '/admin/documents/certificates/' + id);
        ...
        $('.delete-button').on('click', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/documents/certificates/' + id;
