    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Acts List</h3>
                    ...
                    <tbody>
                        @foreach ($acts as $act)
                            <tr>
                                ...
                                <td class="text-center align-middle">{{ $act->name }}</td>
                                <td class="text-center align-middle">{{ $act->description }}</td>
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($act->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $act->id }}"
                                        data-name="{{ $act->name }}"
                                        data-description="{{ $act->description }}"
                                        data-downloadlink="{{ $act->downloadLink }}"
                                        data-visibility="{{ $act->visibility }}"
                                        data-news_n_events="{{ $act->news_n_events }}"
                                        data-new_badge="{{ $act->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $act->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Act</h5>
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Act');
            $('#addUpdateForm').attr('action', '{{ route('acts.store') }}');
        ...
        $(document).on('click', '.update-button', function () {
            ...
            $('#modalTitle').text('Update Act');
            $('#addUpdateForm').attr('action', '/admin/documents/acts/' + id);
        ...
        $(document).on('click', '.delete-button', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/documents/acts/' + id;
