    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Rosters List</h3>
                ...
                <table id="table" class="table display compact table-bordered table-hover" style="width: 100%">
                    <thead>
                        <tr class="table-primary">
                            ...
                            <th class="text-center align-middle">Name of wings</th>
                            <th class="text-center align-middle">Particulars</th>
                            {{-- <th class="text-center align-middle nosort">View</th> --}}
                            <th class="text-center align-middle nosort">Is Header</th>
                        ...
                        @foreach ($rosters as $roster)
                            <tr>
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($roster->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $roster->id }}"
                                        data-name="{{ $roster->name }}"
                                        data-description="{{ $roster->description }}"
                                        data-downloadlink="{{ $roster->downloadLink }}"
                                        data-is_header="{{ $roster->is_header }}"
                                        data-visibility="{{ $roster->visibility }}"
                                        data-news_n_events="{{ $roster->news_n_events }}"
                                        data-new_badge="{{ $roster->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $roster->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Roster</h5>
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <label for="name" class="required">Name of wing:</label>
                                <textarea type="text" class="form-control" id="name" name="name" required></textarea>
                            </div>
                        </div>
                        <div class="col">
                            <div class="form-group">
                                <label for="description" class="required">Particulars (Text for News & Events):</label>
                    ...
                    <div class="row">
                        ...
                        <div class="col">
                            <div class="row">
                                ...
                                <div class="col">
                                    <!-- Header Visibility Checkbox with Hidden Input -->
                                    <div class="form-group">
                                        <input type="hidden" name="is_header" value="0">
                                        <label for="isHeader">Is Header:</label>
                                        <input type="checkbox" class="form-control visibility-toggle" id="isHeader" name="is_header" value="1" data-toggle="toggle" data-on="Yes" data-off="No" data-style="ios" data-onstyle="success" data-offstyle="danger">
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Roster');
            $('#addUpdateForm').attr('action', '{{ route('rosters.store') }}');
        ...
        $(document).on('click', '.update-button', function () {
            ...
            var is_header = $(this).data('is_header') ? true : false;
            var news_n_events = $(this).data('news_n_events') ? true : false;
            var new_badge = $(this).data('new_badge') ? true : false;

            $('#modalTitle').text('Update Roster');
            $('#addUpdateForm').attr('action', '/admin/documents/rosters/' + id);
            ...
            $('#isHeader').prop('checked', is_header).change();
        ...
        $(document).on('click', '.delete-button', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/documents/rosters/' + id;
