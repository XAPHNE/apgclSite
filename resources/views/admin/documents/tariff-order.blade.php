...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Tariff Orders</li>
@endsection
    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Tariff Orders List</h3>
                        ...
                        @foreach ($tariffOrders as $tariffOrder)
                            <tr>
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($tariffOrder->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $tariffOrder->id }}"
                                        data-name="{{ $tariffOrder->name }}"
                                        data-description="{{ $tariffOrder->description }}"
                                        data-downloadlink="{{ $tariffOrder->downloadLink }}"
                                        data-visibility="{{ $tariffOrder->visibility }}"
                                        data-news_n_events="{{ $tariffOrder->news_n_events }}"
                                        data-new_badge="{{ $tariffOrder->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $tariffOrder->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Tariff Order</h5>
...
<div class="modal fade" id="deleteConfirmationModal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this Tariff Order?</p>
            </div>
            <div class="modal-footer justify-content-between">
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Tariff Order');
            $('#addUpdateForm').attr('action', '{{ route('tariff-order.store') }}');
        ...
        $('.update-button').on('click', function () {
            ...
            $('#modalTitle').text('Update Tariff Order');
            $('#addUpdateForm').attr('action', '/admin/documents/tariff-order/' + id);
        ...
        $('.delete-button').on('click', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/documents/tariff-order/' + id;
