...
@section('breadcrumb')
    <li class="breadcrumb-item active">Dam Safety</li>
@endsection
    ...
    <div class="col col-sm-12">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="d-inline">Dam Safety List</h3>
            ...
            <div class="card-body">
                <table id="table" class="table display compact table-bordered table-hover" style="width: 100%">
                    ...
                    <tbody>
                        @foreach ($damSafeties as $damSafety)
                            <tr>
                                <td class="text-center align-middle">{{ $loop->iteration }}</td>
                                <td class="text-center align-middle">{{ $damSafety->name }}</td>
                                <td class="text-center align-middle">{{ $damSafety->description }}</td>
                                <td class="text-center align-middle">
                                    @if ($damSafety->visibility)
                                ...
                                <td class="text-center align-middle" style="white-space: nowrap;">
                                    <a class="btn btn-info" href="{{ url($damSafety->downloadLink) }}" target="_blank"><i title="View/Download" class="fas fa-eye"></i></a>
                                    <button class="btn btn-warning update-button"
                                        data-id="{{ $damSafety->id }}"
                                        data-name="{{ $damSafety->name }}"
                                        data-description="{{ $damSafety->description }}"
                                        data-downloadlink="{{ $damSafety->downloadLink }}"
                                        data-visibility="{{ $damSafety->visibility }}"
                                        data-news_n_events="{{ $damSafety->news_n_events }}"
                                        data-new_badge="{{ $damSafety->new_badge }}"><i title="Update" class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger delete-button"
                                            data-id="{{ $damSafety->id }}"><i title="Delete" class="fas fa-trash-alt"></i>
                                    </button>
                                </td>
                            </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="modalTitle">Add New Dam Safety</h5>
...
<div class="modal fade" id="deleteConfirmationModal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this Dam Safety?</p>
            </div>
            <div class="modal-footer justify-content-between">
...
<script>
    $(document).ready(function () {
        ...
        $('#addButton').on('click', function () {
            $('#modalTitle').text('Add New Dam Safety');
            $('#addUpdateForm').attr('action', '{{ route('dam-safety.store') }}');
        ...
        $(document).on('click', '.update-button', function () {
            ...
            $('#modalTitle').text('Update Dam Safety');
            $('#addUpdateForm').attr('action', '/admin/dam-safety/' + id);
        ...
        $(document).on('click', '.delete-button', function () {
            var id = $(this).data('id');
            var deleteUrl = '/admin/dam-safety/' + id;
